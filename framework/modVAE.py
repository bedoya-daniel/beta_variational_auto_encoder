...
class conv1dVAE(nn.Module):
    ...
    def __init__(self, sound_length=784, out_conv_dim=400, kernel_size=5, h_dim=400, z_dim=5, STRIDE=5):
        """ Creates a VAE net object. 
        ...
        # ENCODER
        self.encoder = nn.Sequential(
            nn.Conv1d(sound_length, out_conv_dim, kernel_size, stride= STRIDE),
            nn.Linear(out_conv_dim, h_dim),
            nn.Sigmoid(),
            nn.Linear(h_dim, z_dim*2))  # 2 for mean and variance.
        ...
        self.decoder = nn.Sequential(
            nn.Linear(z_dim, h_dim),
            nn.Tanh(),
            nn.Linear(h_dim, out_conv_dim),
            nn.Sigmoid(),
            nn.ConvTranspose1d(out_conv_dim, sound_length, kernel_size,stride=STRIDE))
